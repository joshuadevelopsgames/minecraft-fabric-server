...
public class InfernoPearlItem extends Item {
    ...
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        GreekMythologyMod.LOGGER.info("INFERNO PEARL USE METHOD CALLED!");
        ItemStack itemStack = user.getStackInHand(hand);
        
        // Debug logging
        GreekMythologyMod.LOGGER.info("INFERNO PEARL DEBUG: Player {} attempting to use in dimension: {}", 
            user.getName().getString(), world.getRegistryKey().getValue().getPath());
        
        // TEMPORARY: Allow throwing in any dimension for debugging
        if (true) { // world.getRegistryKey().getValue().toString().contains("overworld")) {
            // EXACT copy of vanilla ender pearl mechanics but with custom collision
            GreekMythologyMod.LOGGER.info("INFERNO PEARL DEBUG: In Overworld - proceeding with throw");
            ...
            if (!world.isClient()) {
                // Use EnderPearlEntity but override collision to prevent teleportation
            ...
            GreekMythologyMod.LOGGER.info("INFERNO PEARL DEBUG: Not in Overworld - dimension is: {}", world.getRegistryKey().getValue().getPath());
            if (!world.isClient) {
