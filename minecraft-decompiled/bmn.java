...
public class bmn extends DataFix {
   public bmn(Schema $$0) {
      super($$0, true);
   }

   protected TypeRewriteRule makeRule() {
      Type<? extends Pair<String, ?>> $$0 = this.getInputSchema().getType(blp.A).findFieldType("hoverEvent");
      return this.a(this.getInputSchema().getTypeRaw(blp.A), this.getOutputSchema().getType(blp.A), $$0);
   }
   ...
   private <C1, C2, H extends Pair<String, ?>> TypeRewriteRule a(Type<C1> $$0, Type<C2> $$1, Type<H> $$2) {
      ...
      if (!$$3.equals(this.getInputSchema().getType(blp.A))) {
         throw new IllegalStateException("Text component type did not match, expected " + $$3 + " but got " + this.getInputSchema().getType(blp.A));
      } else {
         Type<?> $$4 = bdt.a($$3, $$3, $$1);
         return this.fixTypeEverywhere("TextComponentHoverAndClickEventFix", $$3, $$1, $$2x -> $$3x -> {
            ...
            return !$$4x ? $$3x : ag.a(bdt.a($$4, $$3x, $$2x), $$1, bmn::a).getValue();
         });
      }
   }

   private static Dynamic<?> a(Dynamic<?> $$0) {
      return $$0.renameAndFixField("hoverEvent", "hover_event", bmn::b).renameAndFixField("clickEvent", "click_event", bmn::c);
   }
   ...
   private static <T> Dynamic<T> c(Dynamic<T> $$0) {
      ...
      return switch ($$1) {
         ...
         case "change_page" -> {
            Integer $$3 = $$0.get("value").result().map(bmn::d).orElse(null);
            if ($$3 == null) {
               yield null;
            } else {
               int $$4 = Math.max($$3, 1);
