import com.google.common.collect.Sets;
import com.mojang.datafixers.util.Pair;
import com.mojang.serialization.MapCodec;
import java.util.Set;
import java.util.function.Predicate;
import java.util.stream.Stream;
import javax.annotation.Nullable;

public class doi extends dob implements dnz.a {
   public static final MapCodec<doi> b = dnx.c.fieldOf("biome").xmap(doi::new, $$0 -> $$0.c).stable();
   private final jl<dnx> c;

   public doi(jl<dnx> $$0) {
      this.c = $$0;
   }

   @Override
   protected Stream<jl<dnx>> b() {
      return Stream.of(this.c);
   }

   @Override
   protected MapCodec<? extends dob> a() {
      return b;
   }

   @Override
   public jl<dnx> getNoiseBiome(int $$0, int $$1, int $$2, dog.f $$3) {
      return this.c;
   }

   @Override
   public jl<dnx> getNoiseBiome(int $$0, int $$1, int $$2) {
      return this.c;
   }

   @Nullable
   @Override
   public Pair<jb, jl<dnx>> a(int $$0, int $$1, int $$2, int $$3, int $$4, Predicate<jl<dnx>> $$5, bck $$6, boolean $$7, dog.f $$8) {
      if ($$5.test(this.c)) {
         return $$7 ? Pair.of(new jb($$0, $$1, $$2), this.c) : Pair.of(new jb($$0 - $$3 + $$6.a($$3 * 2 + 1), $$1, $$2 - $$3 + $$6.a($$3 * 2 + 1)), this.c);
      } else {
         return null;
      }
   }

   @Nullable
   @Override
   public Pair<jb, jl<dnx>> a(jb $$0, int $$1, int $$2, int $$3, Predicate<jl<dnx>> $$4, dog.f $$5, dmx $$6) {
      return $$4.test(this.c) ? Pair.of($$0, this.c) : null;
   }

   @Override
   public Set<jl<dnx>> a(int $$0, int $$1, int $$2, int $$3, dog.f $$4) {
      return Sets.newHashSet(Set.of(this.c));
   }
}
