import it.unimi.dsi.fastutil.objects.ObjectListIterator;
import org.joml.Matrix4f;
import org.joml.Vector4f;

public class hbw implements hbz.a {
   public static final jh[] a = jh.values();
   private final fue b;

   public hbw(fue $$0) {
      this.b = $$0;
   }

   @Override
   public void a(fod $$0, gxn $$1, double $$2, double $$3, double $$4) {
      gxh $$5 = this.b.f;
      if (this.b.C || this.b.D) {
         gye $$6 = $$5.x();
         ObjectListIterator $$27 = $$5.w().iterator();

         while ($$27.hasNext()) {
            hbl.a $$7 = (hbl.a)$$27.next();
            gye.d $$8 = $$6.b($$7);
            if ($$8 != null) {
               jb $$9 = $$7.e();
               $$0.a();
               $$0.a($$9.u() - $$2, $$9.v() - $$3, $$9.w() - $$4);
               Matrix4f $$10 = $$0.c().a();
               if (this.b.C) {
                  fog $$11 = $$1.getBuffer(gxz.v());
                  int $$12 = $$8.b == 0 ? 0 : bcb.g($$8.b / 50.0F, 0.9F, 0.9F);
                  int $$13 = $$12 >> 16 & 0xFF;
                  int $$14 = $$12 >> 8 & 0xFF;
                  int $$15 = $$12 & 0xFF;

                  for (int $$16 = 0; $$16 < a.length; $$16++) {
                     if ($$8.a($$16)) {
                        jh $$17 = a[$$16];
                        $$11.a($$10, 8.0F, 8.0F, 8.0F).a($$13, $$14, $$15, 255).b($$17.j(), $$17.k(), $$17.l());
                        $$11.a($$10, (float)(8 - 16 * $$17.j()), (float)(8 - 16 * $$17.k()), (float)(8 - 16 * $$17.l()))
                           .a($$13, $$14, $$15, 255)
                           .b($$17.j(), $$17.k(), $$17.l());
                     }
                  }
               }

               if (this.b.D && $$7.c().a()) {
                  fog $$18 = $$1.getBuffer(gxz.v());
                  int $$19 = 0;

                  for (jh $$20 : a) {
                     for (jh $$21 : a) {
                        boolean $$22 = $$7.c().a($$20, $$21);
                        if (!$$22) {
                           $$19++;
                           $$18.a($$10, (float)(8 + 8 * $$20.j()), (float)(8 + 8 * $$20.k()), (float)(8 + 8 * $$20.l()))
                              .a(255, 0, 0, 255)
                              .b($$20.j(), $$20.k(), $$20.l());
                           $$18.a($$10, (float)(8 + 8 * $$21.j()), (float)(8 + 8 * $$21.k()), (float)(8 + 8 * $$21.l()))
                              .a(255, 0, 0, 255)
                              .b($$21.j(), $$21.k(), $$21.l());
                        }
                     }
                  }

                  if ($$19 > 0) {
                     fog $$23 = $$1.getBuffer(gxz.z());
                     float $$24 = 0.5F;
                     float $$25 = 0.2F;
                     $$23.a($$10, 0.5F, 15.5F, 0.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 15.5F, 15.5F, 0.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 15.5F, 15.5F, 15.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 0.5F, 15.5F, 15.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 0.5F, 0.5F, 15.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 15.5F, 0.5F, 15.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 15.5F, 0.5F, 0.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 0.5F, 0.5F, 0.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 0.5F, 15.5F, 0.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 0.5F, 15.5F, 15.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 0.5F, 0.5F, 15.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 0.5F, 0.5F, 0.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 15.5F, 0.5F, 0.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 15.5F, 0.5F, 15.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 15.5F, 15.5F, 15.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 15.5F, 15.5F, 0.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 0.5F, 0.5F, 0.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 15.5F, 0.5F, 0.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 15.5F, 15.5F, 0.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 0.5F, 15.5F, 0.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 0.5F, 15.5F, 15.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 15.5F, 15.5F, 15.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 15.5F, 0.5F, 15.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                     $$23.a($$10, 0.5F, 0.5F, 15.5F).a(0.9F, 0.9F, 0.0F, 0.2F);
                  }
               }

               $$0.b();
            }
         }
      }

      hbq $$26 = $$5.y();
      if ($$26 != null) {
         $$0.a();
         $$0.a((float)($$26.b() - $$2), (float)($$26.c() - $$3), (float)($$26.d() - $$4));
         Matrix4f $$27 = $$0.c().a();
         Vector4f[] $$28 = $$26.a();
         fog $$29 = $$1.getBuffer(gxz.z());
         this.a($$29, $$27, $$28, 0, 1, 2, 3, 0, 1, 1);
         this.a($$29, $$27, $$28, 4, 5, 6, 7, 1, 0, 0);
         this.a($$29, $$27, $$28, 0, 1, 5, 4, 1, 1, 0);
         this.a($$29, $$27, $$28, 2, 3, 7, 6, 0, 0, 1);
         this.a($$29, $$27, $$28, 0, 4, 7, 3, 0, 1, 0);
         this.a($$29, $$27, $$28, 1, 5, 6, 2, 1, 0, 1);
         fog $$30 = $$1.getBuffer(gxz.v());
         this.a($$30, $$27, $$28[0]);
         this.a($$30, $$27, $$28[1]);
         this.a($$30, $$27, $$28[1]);
         this.a($$30, $$27, $$28[2]);
         this.a($$30, $$27, $$28[2]);
         this.a($$30, $$27, $$28[3]);
         this.a($$30, $$27, $$28[3]);
         this.a($$30, $$27, $$28[0]);
         this.a($$30, $$27, $$28[4]);
         this.a($$30, $$27, $$28[5]);
         this.a($$30, $$27, $$28[5]);
         this.a($$30, $$27, $$28[6]);
         this.a($$30, $$27, $$28[6]);
         this.a($$30, $$27, $$28[7]);
         this.a($$30, $$27, $$28[7]);
         this.a($$30, $$27, $$28[4]);
         this.a($$30, $$27, $$28[0]);
         this.a($$30, $$27, $$28[4]);
         this.a($$30, $$27, $$28[1]);
         this.a($$30, $$27, $$28[5]);
         this.a($$30, $$27, $$28[2]);
         this.a($$30, $$27, $$28[6]);
         this.a($$30, $$27, $$28[3]);
         this.a($$30, $$27, $$28[7]);
         $$0.b();
      }
   }

   private void a(fog $$0, Matrix4f $$1, Vector4f $$2) {
      $$0.a($$1, $$2.x(), $$2.y(), $$2.z()).a(-16777216).b(0.0F, 0.0F, -1.0F);
   }

   private void a(fog $$0, Matrix4f $$1, Vector4f[] $$2, int $$3, int $$4, int $$5, int $$6, int $$7, int $$8, int $$9) {
      float $$10 = 0.25F;
      $$0.a($$1, $$2[$$3].x(), $$2[$$3].y(), $$2[$$3].z()).a((float)$$7, (float)$$8, (float)$$9, 0.25F);
      $$0.a($$1, $$2[$$4].x(), $$2[$$4].y(), $$2[$$4].z()).a((float)$$7, (float)$$8, (float)$$9, 0.25F);
      $$0.a($$1, $$2[$$5].x(), $$2[$$5].y(), $$2[$$5].z()).a((float)$$7, (float)$$8, (float)$$9, 0.25F);
      $$0.a($$1, $$2[$$6].x(), $$2[$$6].y(), $$2[$$6].z()).a((float)$$7, (float)$$8, (float)$$9, 0.25F);
   }
}
