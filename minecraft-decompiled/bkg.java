...
public class bkg extends DataFix {
   private static final Int2ObjectMap<String> a = ag.a(new Int2ObjectOpenHashMap(), $$0 -> {
   ...
   private static <T> Dynamic<T> a(Dynamic<T> $$0) {
      ...
      Optional<Dynamic<T>> $$1 = $$0.get("HiddenEffect").result().map(bkg::a);
      return $$0.replaceField("HiddenEffect", "hidden_effect", $$1);
   }

   private static <T> Dynamic<T> b(Dynamic<T> $$0, String $$1, String $$2) {
      Optional<Dynamic<T>> $$3 = $$0.get($$1).asStreamOpt().result().map($$1x -> $$0.createList($$1x.map(bkg::a)));
   ...
   private TypeRewriteRule a() {
      Type<?> $$0 = this.getInputSchema().getType(blp.t);
      return this.fixTypeEverywhereTyped("BlockEntityMobEffectIdFix", $$0, $$0x -> this.a($$0x, blp.t, "minecraft:beacon", $$0xx -> {
   ...
   private TypeRewriteRule b() {
      ...
      return this.fixTypeEverywhereTyped("EntityMobEffectIdFix", $$0, $$0x -> {
         $$0x = this.a($$0x, blp.E, "minecraft:mooshroom", bkg::c);
         $$0x = this.a($$0x, blp.E, "minecraft:arrow", bkg::d);
         $$0x = this.a($$0x, blp.E, "minecraft:area_effect_cloud", bkg::e);
         return $$0x.update(DSL.remainderFinder(), bkg::f);
      });
   }

   private TypeRewriteRule c() {
      Type<?> $$0 = this.getInputSchema().getType(blp.c);
      return this.fixTypeEverywhereTyped("PlayerMobEffectIdFix", $$0, $$0x -> $$0x.update(DSL.remainderFinder(), bkg::f));
   }

   private static <T> Dynamic<T> g(Dynamic<T> $$0) {
      Optional<Dynamic<T>> $$1 = $$0.get("Effects").asStreamOpt().result().map($$1x -> $$0.createList($$1x.map(bkg::b)));
      return $$0.replaceField("Effects", "effects", $$1);
   }

   private TypeRewriteRule d() {
      OpticFinder<Pair<String, String>> $$0 = DSL.fieldFinder("id", DSL.named(blp.G.typeName(), bnm.a()));
      Type<?> $$1 = this.getInputSchema().getType(blp.u);
      ...
      return this.fixTypeEverywhereTyped("ItemStackMobEffectIdFix", $$1, $$2x -> {
         ...
         if ($$3.isPresent()) {
            ...
            if ($$4.equals("minecraft:suspicious_stew")) {
               return $$2x.updateTyped($$2, $$0xx -> $$0xx.update(DSL.remainderFinder(), bkg::g));
            }
