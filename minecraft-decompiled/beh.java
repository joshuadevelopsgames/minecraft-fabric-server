...
public class beh extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      return TypeRewriteRule.seq(
         this.fixTypeEverywhereTyped("AttributeIdFix (ItemStack)", $$0, $$1x -> $$1x.updateTyped($$1, $$0xx -> $$0xx.update(DSL.remainderFinder(), beh::b))),
         new TypeRewriteRule[]{
            this.fixTypeEverywhereTyped("AttributeIdFix (Entity)", this.getInputSchema().getType(blp.E), beh::a),
            this.fixTypeEverywhereTyped("AttributeIdFix (Player)", this.getInputSchema().getType(blp.c), beh::a)
         }
      );
   }
   ...
   private static Dynamic<?> b(Dynamic<?> $$0) {
      return $$0.update(
         ...
         $$0x -> $$0x.update("modifiers", $$0xx -> (Dynamic)DataFixUtils.orElse($$0xx.asStreamOpt().result().map(beh::a).map($$0xx::createList), $$0xx))
      );
   }
   ...
   private static Dynamic<?> c(Dynamic<?> $$0) {
      return $$0.renameField("Name", "id")
         ...
         .renameAndFixField(
            "Modifiers",
            "modifiers",
            $$1 -> (Dynamic)DataFixUtils.orElse($$1.asStreamOpt().result().map($$0xx -> $$0xx.map(beh::a)).map(beh::a).map($$0::createList), $$1)
         );
   }
   ...
   private static Typed<?> a(Typed<?> $$0) {
      return $$0.update(
         ...
         $$0x -> $$0x.renameAndFixField(
            "Attributes",
            "attributes",
            $$0xx -> (Dynamic)DataFixUtils.orElse($$0xx.asStreamOpt().result().map($$0xxx -> $$0xxx.map(beh::c)).map($$0xx::createList), $$0xx)
         )
      );
   }
