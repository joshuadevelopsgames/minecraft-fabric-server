...
public class bgx extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      return this.a(this.getInputSchema().getTypeRaw(blp.u), this.getOutputSchema().getTypeRaw(blp.u));
   }

   private <ItemStackOld, ItemStackNew> TypeRewriteRule a(Type<ItemStackOld> $$0, Type<ItemStackNew> $$1) {
      Type<Pair<String, Either<List<ItemStackOld>, Unit>>> $$2 = DSL.named(blp.B.typeName(), DSL.optional(DSL.field("Equipment", DSL.list($$0))));
      ...
      if (!$$2.equals(this.getInputSchema().getType(blp.B))) {
         throw new IllegalStateException("Input entity_equipment type does not match expected");
      } else if (!$$3.equals(this.getOutputSchema().getType(blp.B))) {
         throw new IllegalStateException("Output entity_equipment type does not match expected");
      } else {
         return TypeRewriteRule.seq(
            this.fixTypeEverywhereTyped(
               "EntityEquipmentToArmorAndHandFix - drop chances", this.getInputSchema().getType(blp.E), $$0x -> $$0x.update(DSL.remainderFinder(), bgx::a)
            ),
