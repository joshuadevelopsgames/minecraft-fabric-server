...
public class InfernoPearlItem extends Item implements FabricItem {
    ...
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        ...
        if (world.getRegistryKey().getValue().getPath().equals("overworld")) {
            ...
            // Create and throw a custom snowball entity with inferno pearl behavior
            if (!world.isClient()) {
                SnowballEntity pearlEntity = new SnowballEntity(world, user) {
                    @Override
                    protected void onCollision(HitResult hitResult) {
                        // Custom collision behavior - NO teleportation, just corruption
                        if (!this.getWorld().isClient) {
                            ...
                            for (int i = 0; i < 20; i++) {
                                serverWorld.spawnParticles(ParticleTypes.FLAME, 
                                    impactPos.getX() + this.getWorld().random.nextDouble() - 0.5,
                                    impactPos.getY() + 1 + this.getWorld().random.nextDouble(),
                                    impactPos.getZ() + this.getWorld().random.nextDouble() - 0.5,
                                    1, 0, 0, 0, 0.1);
                            }
                        ...
                        // Remove the entity (no snowball impact effect)
                        this.discard();
                    }
                };
