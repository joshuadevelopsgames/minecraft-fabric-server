...
public class OracleEntity extends IllusionerEntity {
    ...
    // Static methods for command access
    ...
    public void clearQuest(ServerPlayerEntity player) {
        if (activeQuests.containsKey(player)) {
            GodQuest quest = activeQuests.remove(player);
            
            // Send clear message
            player.sendMessage(Text.literal("§6§l[The Oracle] §r§eYour quest has been cleared.").formatted(Formatting.GOLD), false);
            player.sendMessage(Text.literal("§7You can now request a new quest.").formatted(Formatting.GRAY), false);
