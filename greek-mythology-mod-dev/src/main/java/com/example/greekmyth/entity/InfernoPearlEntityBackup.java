...
/**
 * BACKUP CLASS - Preserves the original block-replacement functionality
 * of the Inferno Pearl for future projects.
 * 
 * This class contains the original logic that:
 * - Transforms trees into crimson forest blocks
 * - Corrupts replaceable blocks into nether blocks
 * - Creates a 4-block radius corruption effect
 */
public class InfernoPearlEntityBackup extends ThrownItemEntity {
    
    public InfernoPearlEntityBackup(EntityType<? extends ThrownItemEntity> entityType, World world) {
        super(entityType, world);
    }
    
    public InfernoPearlEntityBackup(World world, LivingEntity owner) {
        super(GreekEntityTypes.INFERNO_PEARL, owner, world, new ItemStack(GreekItems.INFERNO_PEARL));
    }
    
    public InfernoPearlEntityBackup(World world, double x, double y, double z) {
    ...
    protected void onCollision(HitResult hitResult) {
        ...
        if (!this.getWorld().isClient) {
            ...
            GreekMythologyMod.LOGGER.info("INFERNO PEARL BACKUP: Impact at {} - corrupting area", impactPos);
